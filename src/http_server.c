...
static const char index_html[] =
	...
	"@media(prefers-color-scheme:dark){body{background:#111820;color:#edf2f7}.panel,.tableWrap,pre,input,button{background:#17212b;color:#edf2f7;border-color:#3b4b5c}.metric,th{background:#1d2a36;border-color:#3b4b5c}.primary{background:#edf2f7;color:#111820}.hint,.metric span,label{color:#b9c3cf}td,th{border-bottom-color:#273544}}"
	"</style>"
	"</head>"
	"<body>"
	"<main>"
	...
	"</div>"
	"<div class=\"status\"><span>CAN 500 kbit/s</span><span>normal mode</span></div>"
	"</div>"
	...
	"<div class=\"metric\"><span>Errors</span><strong id=\"errCount\">0</strong></div>"
	...
	"<div class=\"wide\"><label for=\"data\">Data bytes</label><input id=\"data\" value=\"01 02\" inputmode=\"text\"><div class=\"hint\">Hex bytes separated by spaces or commas</div></div>"
	"<div class=\"wide checks\"><label><input id=\"ext\" type=\"checkbox\"> Extended ID</label><label><input id=\"rtr\" type=\"checkbox\"> RTR</label></div>"
	"<div class=\"wide toolbar\"><button id=\"send\" class=\"primary\" disabled>Send Frame</button></div>"
	"</div>"
	"</div>"
	"<div>"
	"<div class=\"tableWrap\">"
	...
	"function updateCounts(){rxCountEl.textContent=rxCount;txCountEl.textContent=txCount;errCountEl.textContent=errCount}"
	...
	"function makeFrame(){const data=parseData(dataEl.value);const dlc=Number.parseInt(dlcEl.value,10);if(data.some(v=>!Number.isInteger(v)||v<0||v>255))throw new Error('Invalid data byte');if(!Number.isInteger(dlc)||dlc<0||dlc>8)throw new Error('Invalid DLC');if(!rtrEl.checked&&data.length!==dlc)throw new Error('Data length must match DLC');const id=parseId(idEl.value);if(!Number.isInteger(id)||id<0)throw new Error('Invalid ID');return{bus:0,id,ext:extEl.checked,rtr:rtrEl.checked,dlc,data:rtrEl.checked?[]:data}}"
	"function connect(){"
	"if(ws&&ws.readyState<2)return;"
	"setStatus('wait','connecting');"
	"ws=new WebSocket('ws://'+location.host+'/ws');"
	"ws.onopen=()=>{setStatus('ok','connected');setConnected(true);log('connected')};"
	"ws.onmessage=e=>{log('< '+e.data);try{const msg=JSON.parse(e.data);if(msg.type==='can.rx'){rxCount++;addFrame('RX',msg)}else if(msg.type==='can.tx'){txCount++;addFrame('TX',msg)}else if(msg.type==='error'){errCount++;addFrame('ERR',msg)}updateCounts()}catch(_){}};"
